...
Typical use::
    ...
    from sitekit.answers import answer_questions
...
def _detect_search_use(provider: str, raw: dict) -> bool:
    ...
    -------
    bool
        True if a web-search call (OpenAI/xAI) or search grounding (Gemini)
        appears in the response; False otherwise, including when the
        expected structure is absent.
    """
    try:
        if provider in ("openai", "xai"):
            # Both serve the Responses API; xAI additionally surfaces a
            # top-level ``citations`` list when its search tool fired.
            if any(
